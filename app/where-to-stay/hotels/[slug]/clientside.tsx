...
interface HotelByIdProps extends PropType {
  slug: string;
}

const HotelBySlug: React.FC<HotelByIdProps> = ({ options, slug }) => {
  const API_URL = process.env.NEXT_PUBLIC_ACCOMMODATIONS_API;
  const { data, loading, error } = useFetchBySlug<Hotel>({ slug, API_URL });
