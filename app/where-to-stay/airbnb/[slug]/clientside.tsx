...
interface AirbnbByIdProps extends PropType {
  slug: string;
}

const AirbnbBySlug: React.FC<AirbnbByIdProps> = ({ options, slug }) => {
  const API_URL = process.env.NEXT_PUBLIC_ACCOMMODATIONS_API;
  const { data, loading, error } = useFetchBySlug<Airbnb>({ slug, API_URL });
