...
interface ResortByIdProps extends PropType {
  slug: string;
}

const ResortBySlug: React.FC<ResortByIdProps> = ({ options, slug }) => {
  const API_URL = process.env.NEXT_PUBLIC_ACCOMMODATIONS_API;
  const { data, loading, error } = useFetchBySlug<Resort>({ slug, API_URL });
