...
interface EventsByIdProps extends PropType {
  slug: string;
}

const EventsBySlug: React.FC<EventsByIdProps> = ({ options, slug }) => {
  const API_URL = process.env.NEXT_PUBLIC_EVENTS_API;
  const { data, loading, error } = useFetchBySlug<Events>({ slug, API_URL });
