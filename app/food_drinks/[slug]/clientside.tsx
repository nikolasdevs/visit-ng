...
interface Food_DrinksByIdProps extends PropType {
  slug: string;
}

const Food_DrinksBySlug: React.FC<Food_DrinksByIdProps> = ({
  options,
  slug,
}) => {
  const API_URL = process.env.NEXT_PUBLIC_FOOD_DRINKS_API;
  const { data, loading, error } = useFetchBySlug<Food_Drinks>({
    slug,
    API_URL,
  });
...
export default Food_DrinksBySlug;
