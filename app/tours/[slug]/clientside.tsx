...
interface TourByIdProps extends PropType {
  slug: string;
}

const TourBySlug: React.FC<TourByIdProps> = ({ options, slug }) => {
  const API_URL = process.env.NEXT_PUBLIC_TOURS_API;
  const { data, loading, error } = useFetchBySlug<Tour>({ slug, API_URL });
