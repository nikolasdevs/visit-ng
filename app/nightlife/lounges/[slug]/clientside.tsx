...
interface LoungeByIdProps extends PropType {
  slug: string;
}

const LoungeBySlug: React.FC<LoungeByIdProps> = ({ options, slug }) => {
  const API_URL = process.env.NEXT_PUBLIC_NIGHTLIFE_API;
  const { data, loading, error } = useFetchBySlug<Lounge>({ slug, API_URL });
