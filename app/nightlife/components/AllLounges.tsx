...
export default function AllLounges() {
  ...
  useEffect(() => {
    const fetchAllLounges = async () => {
      try {
        ...
        const response = await axios.get(`${API_URL}/type/lounge`);
        console.log("API Response:", response.data.data); // Log response
        setAllLounges(response.data.data);
      } catch (error) {
        ...
        <div className="flex flex-col gap-8  w-full">
          <div className="  grid gap-8 xl:grid-cols-3 lg:grid-cols-3 md:grid-cols-2 justify-center">
            {allLounges.map((lounge) => (
              ...
              >
                <Link href={`/nightlife/lounges/${lounge.slug}`} className="">
                  <div className="w-full h-64 bg-green-400 relative overflow-hidden">
                    ...
                    >
                      <motion.div
                        ...
                        transition={{ duration: 0.3 }}
                      >
                        {lounge.imageUrls && lounge.imageUrls.length > 0 ? (
                          <Image
                            src={lounge.imageUrls[2]}
                            alt={lounge.name}
                            layout="fill"
                ...
                <div className="pt-2 flex flex-col justify-between h-32 gap-2">
                  <div className="flex flex-col gap-2 ">
                    <Link href={`/nightlife/lounges/${lounge.slug}`}>
                  ...
                  <Link
                    href={`/nightlife/lounges/${lounge.slug}`}
