...
interface MarketByIdProps extends PropType {
  slug: string;
}

const MarketBySlug: React.FC<MarketByIdProps> = ({ options, slug }) => {
  const API_URL = process.env.NEXT_PUBLIC_SHOPPING_API;
  const { data, loading, error } = useFetchBySlug<Market>({ slug, API_URL });
