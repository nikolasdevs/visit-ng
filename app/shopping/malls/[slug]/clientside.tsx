...
interface MallByIdProps extends PropType {
  slug: string;
}

const MallBySlug: React.FC<MallByIdProps> = ({ options, slug }) => {
  const API_URL = process.env.NEXT_PUBLIC_SHOPPING_API;
  const { data, loading, error } = useFetchBySlug<Mall>({ slug, API_URL });
