  ...
  useEffect(() => {
    const fetchAllMalls = async () => {
      try {
        ...
        const response = await axios.get(`${API_URL}/type/mall`);
        console.log("API Response:", response.data.data); // Log response
        setAllMalls(response.data.data);
      } catch (error) {
        ...
        <div className="flex flex-col gap-8  w-full">
          <div className="  grid gap-8 xl:grid-cols-3 lg:grid-cols-3 md:grid-cols-2 justify-center">
            {allMalls.map((mall) => (
              ...
              >
                <Link href={`/shopping/malls/${mall.slug}`} className="">
                  <div className="w-full h-64 relative overflow-hidden">
                    ...
                    >
                      <motion.div
                        ...
                        transition={{ duration: 0.3 }}
                      >
                        {mall.imageUrls && mall.imageUrls.length > 0 ? (
                          <Image
                            src={mall.imageUrls[2]}
                            alt={mall.name}
                            layout="fill"
                ...
                <div className="pt-2 flex flex-col justify-between h-32 gap-2">
                  <div className="flex flex-col gap-2 ">
                    <Link href={`/shopping/malls/${mall.slug}`}>
